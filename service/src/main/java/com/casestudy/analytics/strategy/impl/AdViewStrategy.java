...
@InsightSelector(InsightType.AD_VIEW_COUNT)
public class AdViewStrategy implements InsightStrategy<InsightsResponse> {

    private final InsightRepository insightRepository;

    /**
     * Retrieves insights for the given campaign ID, specifically focusing on ad view counts.
     *
     * @param campaignId the ID of the campaign for which insights are requested
     * @return InsightsResponse containing the view count for the specified campaign
    ...
    public InsightsResponse getInsights(String campaignId) {
        ...
        if (insights == null) {
            throw new InsightNotFoundException(campaignId, InsightType.AD_VIEW_COUNT.name());
        }
